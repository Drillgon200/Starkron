Shader "Custom/CelTextureAnimatedShader"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(Varyings IN) : SV_Target {
				float shadowCutoff = clamp(dot(normalize(IN.normal), -GetMainLight().direction), -1.0, 1.0);
				float shadowBlendFactor = smoothstep(_ShadowAngle, _ShadowAngle + fwidth(shadowCutoff), shadowCutoff);
				half4 baseColor = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, IN.uv) * _BaseColor;
				half4 shadowColor = baseColor * _ShadowColor * SAMPLE_TEXTURE2D(_ShadowColorMap, sampler_ShadowColorMap, IN.uv);
				return half4(lerp(baseColor.rgb, shadowColor.rgb, shadowBlendFactor), 1.0);
			}
			ENDHLSL
		}
		Pass
